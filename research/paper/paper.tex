...
nonzero-exit failure instead of a silent claim of completion, and broke nothing
(\numbbroke{} regressions), at $\nummeancallsB\times$ the model calls of the
baseline. We release the tool, the full raw logs, and the scoring pipeline; every
...
good as the contract, no better. Crucially, the gate is \emph{safe}: \numbbroke{}
tasks regressed relative to baseline.
...
claim of this paper is therefore only ``the gate does no harm'' (\numbbroke{}
regressions); any positive accuracy claim needs a harder benchmark (SWE-bench Lite
...
or an explicit, auditable breach, broke nothing, and cost 12\% extra model calls.
